...
class LemonSqueezyProvider(BillingProvider):
    ...
    async def create_checkout(self, *, company_id, plan_code, success_url, cancel_url) -> CheckoutSession:
        ...
        return CheckoutSession(url=f"https://store.lemonsqueezy.com/checkout/{ref}", provider_ref=ref)

    async def create_subscription(self, *, company_id, plan_code) -> ProviderSubscription:
        return ProviderSubscription(provider_subscription_id=f"ls_{uuid.uuid4().hex[:20]}", status="active")
